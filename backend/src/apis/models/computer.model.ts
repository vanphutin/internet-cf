...
export const ComputerModel = {
  list(
    filters: { status?: ComputerStatus; location?: string; q?: string; limit?: number; offset?: number },
    ...
    if (filters.q) {
      where.push('(name LIKE ? OR ip_address LIKE ?)')
  ...
  findById(id: number, callback: (err: Error | null, row?: Computer) => void) {
    pool.query(
      `SELECT computer_id, name, status, ip_address, location, created_at FROM Computer WHERE computer_id = ?`,
      [id],
      (err, rows: any[]) => {
        if (err) return callback(err)
  ...
  create(
    ...
    pool.query(
      `INSERT INTO Computer (name, status, ip_address, location) VALUES (?, ?, ?, ?)`,
